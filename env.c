...
/* This file defines the OpenMP internal control variables and arranges
   ...
   mqd_t gomp_global_mq = (mqd_t)-1;
   
   static int is_gsched(void) {
       char *env = getenv("IS_GSCHED");
       if (env && strcmp(env, "1") == 0)
           return 1;
       return 0;
   }
   
   /* OpenMP 런타임 초기화 시 호출되는 gsched MQ 초기화 함수 */
   void gomp_init_gsched_mq(void) {
       struct mq_attr attr;
       attr.mq_flags = 0;       /* blocking mode */
       attr.mq_maxmsg = 10;     /* 최대 메시지 개수 */
       attr.mq_msgsize = MAX_MSG_SIZE; /* 최대 메시지 크기 */
       attr.mq_curmsgs = 0;
       
       if (is_gsched()) {
           gomp_global_mq = mq_open(GLOBAL_MQ_NAME, O_CREAT | O_RDONLY, 0644, &attr);
           if (gomp_global_mq == (mqd_t)-1) {
               perror("gsched: mq_open failed");
               exit(EXIT_FAILURE);
           }
           printf("gsched: Global MQ '%s' created (O_RDONLY) for receiving task messages.\n", GLOBAL_MQ_NAME);
       } else {
           gomp_global_mq = mq_open(GLOBAL_MQ_NAME, O_WRONLY);
           if (gomp_global_mq == (mqd_t)-1) {
               perror("normal program: mq_open failed");
               
               return;
           }
           char prog_name[256] = "my_program";  
           if (mq_send(gomp_global_mq, prog_name, strlen(prog_name) + 1, 0) == -1) {
               perror("normal program: mq_send failed");
           } else {
               printf("normal program: Sent program name '%s' to gsched via MQ '%s'.\n", prog_name, GLOBAL_MQ_NAME);
           }
           mq_close(gomp_global_mq);
           gomp_global_mq = (mqd_t)-1;
       }
   }
   //GSCHED_OMP_E
   ...
   initialize_env (void)
   {
     ...
     //environ은 환경 변수 리스트고, 여기서 OMP_ 환경 변수를 식별해서 그에 맞게 처리
     //하지만 기본적으로는 설정 안되어 있음
     if (environ)
     ...
     // 위에서 환경 변수를 통해 ALL 설정을 하지 않았다면,
     // 리스트 검색 내용이 없으므로 all = NULL이다.
     all = gomp_get_initial_icv_item (GOMP_DEVICE_NUM_FOR_ALL);
     for (omp_var = 0; omp_var < OMP_HOST_VAR_CNT; omp_var++)
       {
         // host_envvars는 미리 초기화된 정보이고 icv list의 플래그와 일치하면 처리
         // 하지만 위에서 만들 때 따로 flag를 설정은 안하지만 만들 때 0 값이 채워져있다.
         // 결과적으로는 일치하는게 하나도 없어서 해당 설정 패스
         if (none != NULL // none은 무조건 초기화 되어 있으므로 존재한다.
     ...
     // 해당 조건은 모두 만족해서 icvs.default_device_var은 0으로 초기화됨.
